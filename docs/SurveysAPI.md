...
------------- | ------------- | -------------
[**GetSurveysIdSurveyQuestions**](SurveysAPI.md#GetSurveysIdSurveyQuestions) | **Get** /v1/surveys/{id}/survey_questions | List questions for a survey



## GetSurveysIdSurveyQuestions

> GetSurveysIdSurveyQuestions(ctx, id).Authorization(authorization).X4meAccount(x4meAccount).XXurrentLanguage(xXurrentLanguage).PerPage(perPage).SearchAfter(searchAfter).SearchBefore(searchBefore).Fields(fields).Sort(sort).State(state).Execute()
...
func main() {
	...
	r, err := apiClient.SurveysAPI.GetSurveysIdSurveyQuestions(context.Background(), id).Authorization(authorization).X4meAccount(x4meAccount).XXurrentLanguage(xXurrentLanguage).PerPage(perPage).SearchAfter(searchAfter).SearchBefore(searchBefore).Fields(fields).Sort(sort).State(state).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SurveysAPI.GetSurveysIdSurveyQuestions``: %v\n", err)
...
Other parameters are passed through a pointer to a apiGetSurveysIdSurveyQuestionsRequest struct via the builder pattern
