# \FirstLineSupportAgreementsAPI

All URIs are relative to *https://api.xurrent.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**GetFlsasIdAudit**](FirstLineSupportAgreementsAPI.md#GetFlsasIdAudit) | **Get** /v1/flsas/{id}/audit | List audit entries for a first line support agreement



## GetFlsasIdAudit

> GetFlsasIdAudit(ctx, id).Authorization(authorization).X4meAccount(x4meAccount).XXurrentLanguage(xXurrentLanguage).PerPage(perPage).SearchAfter(searchAfter).SearchBefore(searchBefore).Fields(fields).Sort(sort).State(state).Execute()

List audit entries for a first line support agreement
...
func main() {
	...
	r, err := apiClient.FirstLineSupportAgreementsAPI.GetFlsasIdAudit(context.Background(), id).Authorization(authorization).X4meAccount(x4meAccount).XXurrentLanguage(xXurrentLanguage).PerPage(perPage).SearchAfter(searchAfter).SearchBefore(searchBefore).Fields(fields).Sort(sort).State(state).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `FirstLineSupportAgreementsAPI.GetFlsasIdAudit``: %v\n", err)
...
Other parameters are passed through a pointer to a apiGetFlsasIdAuditRequest struct via the builder pattern
