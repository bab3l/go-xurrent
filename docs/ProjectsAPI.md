...
------------- | ------------- | -------------
[**GetProjectsIdAudit**](ProjectsAPI.md#GetProjectsIdAudit) | **Get** /v1/projects/{id}/audit | List audit entries for a project



## GetProjectsIdAudit

> GetProjectsIdAudit(ctx, id).Authorization(authorization).X4meAccount(x4meAccount).XXurrentLanguage(xXurrentLanguage).PerPage(perPage).SearchAfter(searchAfter).SearchBefore(searchBefore).Fields(fields).Sort(sort).State(state).Execute()
...
func main() {
	...
	r, err := apiClient.ProjectsAPI.GetProjectsIdAudit(context.Background(), id).Authorization(authorization).X4meAccount(x4meAccount).XXurrentLanguage(xXurrentLanguage).PerPage(perPage).SearchAfter(searchAfter).SearchBefore(searchBefore).Fields(fields).Sort(sort).State(state).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ProjectsAPI.GetProjectsIdAudit``: %v\n", err)
...
Other parameters are passed through a pointer to a apiGetProjectsIdAuditRequest struct via the builder pattern
